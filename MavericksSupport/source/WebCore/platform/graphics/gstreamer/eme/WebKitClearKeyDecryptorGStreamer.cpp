...
struct WebKitMediaClearKeyDecryptPrivate {
    // GstBaseTransform serializes samples per decryptor; audio/video have separate cipher state.
    PAL::GCrypt::Handle<gcry_cipher_hd_t> cipher;
    ...
    Lock lock;
    RefPtr<CDMProxyClearKey> cdmProxy WTF_GUARDED_BY_LOCK(lock);
};
...
static bool cdmProxyAttached(WebKitMediaCommonEncryptionDecrypt* self, const RefPtr<CDMProxy>& cdmProxy)
{
    ...
    // The drm-cdm-proxy context carries whichever CDM the page created, and the Widevine decryptor
