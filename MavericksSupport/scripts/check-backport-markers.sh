...
#      (upstream/safari-7625.1.29.11-branch by default;
#       override via $MAVERICKS_UPSTREAM_REF)
#
# The baseline follows upstream revisions incorporated into committed history.
# During an uncommitted merge, pass the intended upstream revision explicitly.
#
...
UPSTREAM_REF_CANDIDATES="${MAVERICKS_UPSTREAM_REF:-upstream/safari-7625.1.29.11-branch}"
...
if [ "$#" -ge 1 ] && [ -n "${1:-}" ]; then
    ...
    if [ -z "$BASE" ]; then
        echo "ERROR: no shared upstream base found; set MAVERICKS_UPSTREAM_BASE or MAVERICKS_UPSTREAM_REF." >&2
        exit 2
    fi
fi
...
: > "$GENERATED_TXT"
git diff --diff-filter=ACMR --name-only -z "$BASE" -- Source/ 2>/dev/null | \
xargs -0 awk '
    FNR <= 3 && tolower($0) ~ /generated file: do not edit|do not edit this file|auto-generated|automatically generated/ {
...
xargs -0 awk -v MARKER="$MARKER" -v GENF="$GENERATED_TXT" '
    BEGIN {
        while ((getline generated < GENF) > 0) if (generated != "") gen[generated] = 1
        close(GENF)
    }
    function excluded(p) {
        if (p ~ /(^|\/)MavericksSupport\// || p ~ /(^|\/)WebKitBuild\// ||
            p ~ /\/DerivedSources\// || p ~ /\/Derived\// ||
            p ~ /^Source\/ThirdParty\/[^\/]+\/lib\// ||
            p ~ /\.(a|o|dylib|so|bin|dat|png|jpg|jpeg|gif|bmp|ico|icns|ttf|otf|woff|woff2|pdf|zip|gz|tar|mov|mp4|webp|wasm)$/ ||
            p ~ /\.json$/) return 1
        return 0
    }
    function report(    text) {
        if (file == "" || skip || has_marker) return
        text = snippet
    ...
    FNR == 1 {
        report()
        file = FILENAME
        skip = excluded(file) || (file in gen)
        has_marker = 0
        snippet = ""
    }
    {
        if (!skip && index($0, MARKER) > 0) has_marker = 1
        if (!skip && snippet == "" && $0 ~ /[^ \t\r]/) snippet = $0
    }
    END { report() }
' >> "$OUT_NEWFILE"
...
MARKER_LINES="$TMPDIR_RUN/marker-lines"
# Marker lines are indexed once, so each hunk's context check is a lookup.
git diff --diff-filter=ACMR --name-only -z "$BASE" -- Source/ 2>/dev/null | \
xargs -0 grep -n -H -F "$MARKER" -- > "$MARKER_LINES" 2>/dev/null || true
git diff -U0 "$BASE" -- Source/ 2>/dev/null | awk \
    -v MARKER="$MARKER" \
    -v CTXN="$CONTEXT_ABOVE" \
    -v GENF="$GENERATED_TXT" \
    -v MARKF="$MARKER_LINES" '
BEGIN {
    if (GENF != "") {
    ...
    if (MARKF != "") {
        while ((getline ml < MARKF) > 0) {
            split(ml, ma, ":")
            if (ma[1] != "" && ma[2] != "") marker_line[ma[1], ma[2] + 0] = 1
        }
        close(MARKF)
    }
}
...
function ctx_above(f, ln,    i, start, end) {
    if (f == "" || ln <= 1) return ""
    start = ln - CTXN; if (start < 1) start = 1
    end = ln - 1
    for (i = start; i <= end; i++) {
        if ((f, i) in marker_line) return MARKER
    }
    return ""
}
