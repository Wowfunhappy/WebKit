...
GST_SRC="$REPO/MavericksSupport/deps/build/lib"   # the GStreamer runtime build_deps.sh builds from source
...
if [ -d "$GST_SRC" ]; then
    ...
    # every dylib and plugin targets 10.9 directly, so deployment is a straight copy.
    mkdir -p "$GST_DEPLOY"
