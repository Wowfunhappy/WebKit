...
// ---------------------------------------------------------------------------------------------------
// -[NSURLProtectionSpace _webKitPropertyListData] / -_initWithWebKitPropertyListData: (15.4+): a
// protection space as a property list, and the initializer that reverses it. WebKit's IPC layer uses
// this pair when an object has it and falls back to NSCoding otherwise (conformsToWebKitSecureCoding in
// CoreIPCSecureCoding.mm), so the two have to agree on keys with that fallback. They do, because they
// are the same operation: 10.9's -encodeWithCoder: encodes exactly one key,
// __nsurlprotectionspace_proto_plist, holding what _CFURLProtectionSpaceCreateArchive returns, and
// -initWithCoder: reads that key back through _CFURLProtectionSpaceCreateFromArchive. So these call
// that CFNetwork pair directly and put its dictionary under the same key: same plist, same wire bytes
// the NSCoding path already produces, and the server trust (which nothing else here can carry across a
// property list) survives.
//
// The property list is CACHED on the space, which is the reason to supply this pair at all. A
// protection space is immutable, but archiving one on 10.9 runs a full SecTrustEvaluate inside
// CFNetwork's archiver, which stores the evaluated chain — and evaluating a real-world chain costs
// ~0.5s of RSA work warm, seconds cold, because ocspd fetches OCSP responses and CRLs for it.
// SecTrustEvaluate repeats all of that on every call (it keeps no result in the trust), so a second
// archive of the same space costs the same as the first: measured 0.478s then 0.474s. Modern macOS
// hands WebKit a cheap accessor here; caching makes this one behave the same, which is what lets
// NetworkSessionCocoa produce the representation off the queue that delivers every NSURLSession
// callback instead of on it (github #116).
typedef const struct _CFURLProtectionSpace *WKCFURLProtectionSpaceRef;
// CFNetwork SPI, exported on 10.9 but not declared in any public header; both take the allocator first,
// per the CF convention and confirmed on-host (calling them without it faults).
extern CFDictionaryRef _CFURLProtectionSpaceCreateArchive(CFAllocatorRef, WKCFURLProtectionSpaceRef);
extern WKCFURLProtectionSpaceRef _CFURLProtectionSpaceCreateFromArchive(CFAllocatorRef, CFDictionaryRef);

static NSString * const kWKProtectionSpaceArchiveKey = @"__nsurlprotectionspace_proto_plist";
static const char kWKProtectionSpacePropertyListKey;

@interface NSURLProtectionSpace (WKMavericksSPI)
// 10.9's own bridge between NSURLProtectionSpace and the CFURLProtectionSpace it wraps (the second is
// spelled with Apple's typo). -_initWithCFURLProtectionSpace: retains its argument, so a space handed
// to it straight from a Create function has to be released after.
- (WKCFURLProtectionSpaceRef)_cfurlprtotectionspace;
- (instancetype)_initWithCFURLProtectionSpace:(WKCFURLProtectionSpaceRef)space;
@end

@interface NSURLProtectionSpace (WKPolyfillScope)
- (NSDictionary *)wk__webKitPropertyListData;
- (instancetype)wk__initWithWebKitPropertyListData:(NSDictionary *)propertyList;
@end
@implementation NSURLProtectionSpace (WKPolyfillScope)
- (NSDictionary *)wk__webKitPropertyListData
{
    // The lock is held across the archiving so two callers cannot both pay for it. A protection space
    // belongs to one connection, so this contends with nothing.
    @synchronized (self) {
        NSDictionary *cached = objc_getAssociatedObject(self, &kWKProtectionSpacePropertyListKey);
        if (cached)
            return cached;
        WKCFURLProtectionSpaceRef space = [self _cfurlprtotectionspace];
        CFDictionaryRef archive = space ? _CFURLProtectionSpaceCreateArchive(kCFAllocatorDefault, space) : NULL;
        if (!archive)
            return nil;
        NSDictionary *propertyList = @{ kWKProtectionSpaceArchiveKey: (id)archive };
        CFRelease(archive);
        objc_setAssociatedObject(self, &kWKProtectionSpacePropertyListKey, propertyList, OBJC_ASSOCIATION_RETAIN);
        return propertyList;
    }
}
- (instancetype)wk__initWithWebKitPropertyListData:(NSDictionary *)propertyList
{
    NSDictionary *archive = [propertyList objectForKey:kWKProtectionSpaceArchiveKey];
    if (![archive isKindOfClass:[NSDictionary class]]) {
        [self release];
        return nil;
    }
    WKCFURLProtectionSpaceRef space = _CFURLProtectionSpaceCreateFromArchive(kCFAllocatorDefault, (CFDictionaryRef)archive);
    if (!space) {
        [self release];
        return nil;
    }
    self = [self _initWithCFURLProtectionSpace:space];
    CFRelease(space);
    // This space came from that property list, so the property list is also the accessor's answer:
    // keeping it means a space that crossed IPC and gets re-encoded does not archive itself again.
    // An immutable copy, because the caller's dictionary is its own to mutate afterwards (WebKit's
    // own decoder builds an NSMutableDictionary) and the accessor must keep answering with the
    // property list this space was built from.
    if (self)
        objc_setAssociatedObject(self, &kWKProtectionSpacePropertyListKey, [[propertyList copy] autorelease], OBJC_ASSOCIATION_RETAIN);
    return self;
}
@end
WK_POLYFILL_SEL("_webKitPropertyListData", "wk__webKitPropertyListData");
WK_POLYFILL_SEL("_initWithWebKitPropertyListData:", "wk__initWithWebKitPropertyListData:");
