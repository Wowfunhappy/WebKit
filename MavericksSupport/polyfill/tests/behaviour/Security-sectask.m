// The SecTaskCopySigningIdentifier / SecTaskGetCodeSignStatus polyfills (polyfills/c/Security.c):
...
// call the polyfill, exactly as polyfills/c/Security.c has to.
...
int main(void)
{
    @autoreleasepool {
        ...
        if (failures) {
            printf("Security-sectask: %d check(s) FAILED\n", failures);
            return 1;
        }
        printf("Security-sectask: all checks passed\n");
    }
    return 0;
}
