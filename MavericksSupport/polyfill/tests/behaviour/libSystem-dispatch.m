// The dispatch_activate polyfill (polyfills/c/libSystem.m, over the rule in
...
int main(void)
{
    @autoreleasepool {
        ...
        if (failures) {
            printf("libSystem-dispatch: %d check(s) FAILED\n", failures);
            return 1;
        }
        printf("libSystem-dispatch: all checks passed\n");
    }
    return 0;
}
