...
# for the vendored GStreamer dylibs; done by install-safari7.sh for the WebKit frameworks). Because the shim
...
# polyfill/scripts/build-polyfill.sh (libpolyfill_classes.dylib). The matching install-side repoint helper
# lives in install-safari7.sh (repoint_framework_dep).
#
# Usage:
