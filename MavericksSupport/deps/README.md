...
`build_deps.sh` compiles through **its own ccache** in `build/ccache` (1 GB cap, gitignored with the
rest of `build/`) — separate from the WebKit build's much larger cache in `WebKitBuild/ccache` so
neither can evict the other. Because each run builds in a fresh `mktemp -d`, the script sets
`CCACHE_BASEDIR`/`CCACHE_NOHASHDIR` so objects still hit across runs. `MAVERICKS_CCACHE` overrides the
binary; if none is executable the deps build simply compiles uncached. A normal rerun keeps the cache
(the script only clears `build/{include,lib,bin}`); the `rm -rf build` in *Updating* below discards it
too, which just costs one uncached rebuild.
