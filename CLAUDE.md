...
- **The moment your edit batch is ready, `pkill -x ninja` and relaunch immediately.** The last tool call of any edit batch that changes built source is the kill + relaunch. No judgment required, no size threshold.
...
- **Never run two builds at once.** Two concurrent `rebuild.sh` runs produce ~51 bogus `no such file or directory: '@…UnifiedSource-….o.rsp'` errors; none of them are real.
- **`pkill -x ninja` only, never `pkill -9`**, and only once ninja actually exists. rebuild.sh has a cmake phase first; killing during it destroys `build.ninja` and every later build dies instantly with `ninja: error: loading 'build.ninja'`. Recovery is cheap: `cmake .` in `WebKitBuild/Release` — never wipe the build dir.
